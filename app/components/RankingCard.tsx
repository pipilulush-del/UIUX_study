...
import { Bookmark, BookmarkCheck, CirclePlay, Newspaper, MessageSquare } from "lucide-react";
...
export default function RankingCard({ item, isSaved, onToggleSave }: Props) {
  const SourceIcon = SOURCE_ICONS[item.source];
  const rankStr = String(item.rank).padStart(2, "0");

  return (
    <div className="flex items-start gap-4 px-6 lg:px-12 py-5 border-b border-white/[0.08] hover:bg-white/[0.02] transition-colors duration-150 group">
      {/* Rank badge */}
      <span className="font-mono text-xs border border-white/20 px-2 py-1 shrink-0 mt-0.5 leading-none text-white tabular-nums">
        {rankStr}
      </span>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            className={`shrink-0 transition-colors duration-150 mt-0.5 ${
              isSaved ? "text-white" : "text-white/30 hover:text-white/70"
            }`}
